...
public class OrganisationAPI {
    ...
    private static Logger logger = Logger.getLogger( OrganisationAPI.class.getName() );

    @PUT
    public Response create( @PathParam( "organisation" ) String organisationName, @CookieParam( "session" ) String session ) {

        logger.info( "Creating the organisation " + organisationName );

        User user = Rut.getInstance().authenticate( session );
        if( !Rut.getInstance().isAllowed( user, Rut.getInstance(), Permission.WRITE ) ) {
            return Response.status( Response.Status.UNAUTHORIZED ).entity( user + " is not allowed to create organisations" ).build();
        }
        ...
        if( organisationName == null || organisationName.isEmpty() ) {
            ...
            Organisation organisation = Organisation.create( organisationName );
            Rut.getInstance().getOrganisations().put( organisationName, organisation );
            return Response.status( 201 ).entity( "/organisation/" + organisation.getName() ).build();
        }
    }
